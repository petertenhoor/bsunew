...
/**
 * Class JobPostType
 * @package Bsu\PostType
 */
class JobPostType extends AbstractPostType
{
    const TYPE = 'vacatures';
    const BASE_SLUG = 'vacatures';

    /**
     * AgendaPostType constructor.
     */
    protected function __construct()
    {
        ...
        $this->setLabelTranslation(
            __('Job', BsuTheme::TEXTDOMAIN),
            __('Jobs', BsuTheme::TEXTDOMAIN)
        );

        // set default post type supports
        $this->setSupports(
            array(
                self::SUPPORT_TITLE,
                self::SUPPORT_EDITOR,
                self::SUPPORT_THUMBNAIL,
                self::SUPPORT_EXCERPT
            )
        );

        add_action('add_meta_boxes', array($this, 'addSalientHeaderOptions'));

        // set icon
        $this->setIcon('dashicons-hammer');

        // force using visual composer
        $this->forceUseVisualComposer();

        // register post type
        $this->registerPostType(
            array(
                'description' => __('BSU vacatures', BsuTheme::TEXTDOMAIN),
                'has_archive' => true,
                'rewrite'     => array(
    ...
    /**
     * Add Salient header options metabox
     *
     */
    public function addSalientHeaderOptions()
    {

        #-----------------------------------------------------------------#
        # Salient Header Settings
        #-----------------------------------------------------------------#
        ...
        $meta_box = array(
            ...
            'title'       => __('Vacature Header Settings', NECTAR_THEME_NAME),
            ...
            'fields'      => array(
                ...
                array(
                    'name' => __('Page Header Shortcode', NECTAR_THEME_NAME),
                    'desc' => __('(optional)', NECTAR_THEME_NAME),
                    'id'   => '_nectar_header_shortcode',
                    'type' => 'textarea',
                    'std'  => ''
                ),
                array(
